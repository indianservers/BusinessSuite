...
MODULE_WIDGETS = {
    ...
    "inventory": ["Stock Value", "Low Stock", "COGS", "GRNI", "Valuation", "HSN Summary", "Gross Margin"],
...
REPORTS = [
    ...
    ReportDefinition("inventory_valuation", "Inventory Valuation", ("inventory",), "Requires Inventory."),
    ...
    ReportDefinition("stock_cogs", "Stock COGS", ("inventory", "fam"), "Requires Inventory + FAM."),
]

ROLE_CATALOG = [
    {"role": "Accounts Admin", "modules": ["fam"], "permissions": ["fam_view", "fam_manage", "fam_admin", "fam_vouchers_post", "fam_gst_view"]},
    {"role": "Inventory Manager", "modules": ["inventory"], "permissions": ["fam_inventory_view", "fam_inventory_manage", "fam_stock_post"]},
...
AI_TOPICS = {
    ...
    "inventory": {"stock", "inventory", "warehouse", "grn", "cogs", "hsn"},
    "srm": {"sales order", "contract", "billing", "invoice", "collection", "revenue"},
...
def dashboard_widgets(enabled: set[str]) -> list[dict[str, str]]:
    ...
    elif has(enabled, "fam", "inventory") and not ({"crm", "srm", "project_management"} & enabled):
        widgets.extend(MODULE_WIDGETS["fam"] + MODULE_WIDGETS["inventory"])
    ...
    elif has(enabled, "fam", "inventory", "crm", "srm", "project_management"):
        widgets.extend(MODULE_WIDGETS["full"])
    else:
        if "fam" in enabled:
            widgets.extend(MODULE_WIDGETS["fam"])
        if "inventory" in enabled:
            widgets.extend(MODULE_WIDGETS["inventory"])
...
def customer_720_sections(enabled: set[str]) -> list[dict[str, str]]:
    ...
    mapping = [
        ...
        ("srm", "SRM", "Sales orders, contracts, billing plans, invoices and collections."),
        ...
        ("inventory", "Inventory", "Stock reservations, dispatches, returns and COGS traces."),
    ]
...
def ai_answer(enabled: set[str], question: str) -> dict[str, object]:
    ...
    if matched_module and matched_module not in enabled:
        labels = {"crm": "CRM", "inventory": "Inventory", "srm": "SRM", "project_management": "PMS", "fam": "FAM"}
        return {
            "allowed": False,
